...
public class CustomSystem : MonoBehaviour
{
    ...
    public void Stop()
    {
        ...
        for (int i = 0; i < activeTrainInfos.Count; i++)
        {
            NormalNode currentNode = activeTrainInfos[i].activeTrain.CurrentNode;
            currentNode.ClearTrain();
        }

        for (int i = 0; i < activeTrainInfos.Count; i++)
        {
            NormalNode startNode = activeTrainInfos[i].startNode;
            Train train = activeTrainInfos[i].activeTrain;
        ...
        {
            NormalNode startNode = activeStationInfos[i].startNode;
    ...
    public void Restart()
    {
        foreach(NodeSlot slot in disableNodeSlots)
        {
            ...
            {
                NormalNode normalNode = Instantiate(pfNode, node.transform.position, Quaternion.identity, nodeParent).GetComponent<NormalNode>();
